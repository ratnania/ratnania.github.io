...
class Grad(Function):
    ...
    def eval(cls, expr):
        ...
        if isinstance(expr, Mul):
            ...
            if isinstance(left, VectorFunction):
                return Dot(left, d_right) + right * d_left
            elif isinstance(right, VectorFunction):
                return left * d_right + Dot(right, d_left)
            else:
...
class Dot(Function):
    ...
    def eval(cls, arg1, arg2):
        ...
        args_1 = [i for i in a if isinstance(i, (VectorFunction, Grad))]
        c1     = [i for i in a if not i in args_1]
        args_2 = [i for i in b if isinstance(i, (VectorFunction, Grad))]
...
def test_grad_2():
    ...
    assert(Grad(u*phi) == Grad(u)*phi + Dot(Grad(phi), u))
    assert(Grad(phi*u) == Grad(u)*phi + Dot(Grad(phi), u))
